import{g as j,a as w,s as S,h as $,b as I,c as W,w as v}from"./assets/burgerMenu-362f790e.js";import{N as C,a as A,b as O}from"./assets/vendor-36465543.js";const h=document.querySelector(".list_general"),R=document.querySelector(".title_best_sellers");let g=0,i=window.innerWidth;window.addEventListener("resize",H);function H(e){const t=e.target.outerWidth;(t>767&&i<768||t>1439&&i<1440||t<1440&&i>1439||t<768&&i>767)&&location.reload()}const K=()=>{i<768?g=1:i>=768&&i<1440?g=3:g=5};j().then(e=>{console.log(e),Y(e)}).catch(e=>{C.Notify.failure("An error occurred while fetching images. Please try again.")});function z(){let e=this.getAttribute("data-my-attribute");console.log(e),P(e),w(e).then(t=>{R.style.display="none",h.innerHTML="",T(t)}).catch(t=>{C.Notify.failure("An error occurred while fetching images. Please try again.")})}function P(e){const t=document.querySelector(".categories__title"),o=document.querySelector(".categories__list").getElementsByTagName("li"),d=Array.from(o).find(b=>b.textContent.trim()===e);d&&(d.classList.add("selected-category"),t.style.color="var(--text-color)")}function D(e){return e.books.slice(0,g).map(t=>{let s=q(t),o=x(t);return`
  ...
  <button class="btn_back">BACK</button></div>`}function T(e){window.scrollTo(0,0);const t=F(e),s=J(e),o=G(s,t);h.insertAdjacentHTML("beforeend",o),document.querySelector(".btn_back").addEventListener("click",V)}function V(){location.reload()}K();const c={galleryTitle:document.querySelector(".list_general-title"),filterListEl:document.querySelector(".categories__list"),categoriesTitle:document.querySelector(".categories__title"),title:document.querySelector(".title_best_sellers")};S();function Q(e){I().then(t=>{const s=t.data;s.sort((o,r)=>o.list_name.localeCompare(r.list_name)),X(s)})}function X(e){const t=e.map(s=>`<li class="categories__list-item">${s.list_name}</li>`);c.filterListEl.insertAdjacentHTML("beforeend",t.join("")),$()}Q();c.filterListEl.addEventListener("click",Z);function Z(e){ee(e),S();const t=e.target.textContent;w(t).then(s=>{h.innerHTML="",T(s),$(),c.title.style.display="none"}).catch(s=>{Notiflix.Notify.failure("An error occurred while fetching images. Please try again.")})}function ee(e){if(e.target.tagName==="LI"){const t=c.filterListEl.querySelector(".selected-category");t&&t.classList.remove("selected-category"),e.target.classList.add("selected-category")}c.categoriesTitle.style.color="var(--text-color)"}A.defaults.baseURL="https://books-backend.p.goit.global/books/";async function L(e){const{data:t}=await A(e);return console.log(t),t}const E=document.querySelector("body"),l=[];document.addEventListener("click",te);function te(e){if(console.log(e),!e.target.closest(".modal-list"))return;console.log(e.target),e.preventDefault();const s=e.target.closest(".modal-list").dataset.id;console.log(s),L(s).then(o=>{console.log(l);const{_id:r,author:d,book_image:b,title:k,description:B,buy_links:[{url:M},{url:N}]}=o,p=O.create(`
   <div class="modal-book-js" data-id="${r}">
          ...
          <img src="${b}" alt="${k}" class="modal-img-book widht="192" height="281"/>
    ...
    </div>`,{onShow:()=>{document.addEventListener("keydown",_),E.style.overflow="hidden"},onClose:()=>{document.removeEventListener("keydown",_),E.style.overflow="auto"}});function _(m){const f="Escape";m.code===f&&p.close()}p.show(),document.querySelector(".modal-book-btn-close").addEventListener("click",()=>{p.close()});const a=document.querySelector("#add"),n=document.querySelector("#remove"),u=document.querySelector(".txt-remove");if(!W())a.classList.add("hidden"),n.classList.add("hidden"),u.classList.add("hidden");else{const m="booklist";l.push(o),v(l),console.log(l),L(s).then(f=>{const y=f.id;localStorage.getItem({idBook:y})!==null?(a.classList.add("hidden"),n.classList.remove("hidden")):(a.classList.remove("hidden"),n.classList.add("hidden"),u.classList.add("hidden")),a.addEventListener("click",()=>{localStorage.setItem(m,JSON.stringify(l)),a.classList.add("hidden"),n.classList.remove("hidden"),u.classList.remove("hidden")}),n.addEventListener("click",()=>{localStorage.removeItem(m,JSON.stringify(l)),v(l),a.classList.remove("hidden"),n.classList.add("hidden"),u.classList.add("hidden")})})}})}
