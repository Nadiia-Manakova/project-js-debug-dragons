import{g as N,a as v,s as L,h as E,b as j,c as R,w as I}from"./assets/burgerMenu-8b96f2f2.js";import{N as w,a as S,b as W}from"./assets/vendor-3361d7a0.js";const g=document.querySelector(".list_general"),O=document.querySelector(".title_best_sellers");let m=0,i=window.innerWidth;window.addEventListener("resize",H);function H(e){const t=e.target.outerWidth;(t>767&&i<768||t>1439&&i<1440||t<1440&&i>1439||t<768&&i>767)&&location.reload()}const K=()=>{i<768?m=1:i>=768&&i<1440?m=3:m=5};N().then(e=>{console.log(e),Y(e)}).catch(e=>{w.Notify.failure("An error occurred while fetching images. Please try again.")});function z(){let e=this.getAttribute("data-my-attribute");console.log(e),P(e),v(e).then(t=>{O.style.display="none",g.innerHTML="",C(t)}).catch(t=>{w.Notify.failure("An error occurred while fetching images. Please try again.")})}function P(e){const t=document.querySelector(".categories__title"),o=document.querySelector(".categories__list").getElementsByTagName("li"),d=Array.from(o).find(h=>h.textContent.trim()===e);d&&(d.classList.add("selected-category"),t.style.color="var(--text-color)")}function D(e){return e.books.slice(0,m).map(t=>{let s=$(t),o=A(t);return`
    ...
    `}).join("")}function Y(e){window.scrollTo(0,0);const t=U(e);g.insertAdjacentHTML("beforeend",t),document.querySelectorAll(".btn").forEach(o=>{o.addEventListener("click",z)})}function J(e){return e.slice(0,1).map(t=>t.list_name.split(" ").map(s=>`<span>${s}</span>`).join(" "))}function $(e){return e.title.length>16?e.title.substring(0,16)+"...":e.title}function A(e){return e.author.length>25?e.author.substring(0,20)+"...":e.author}function F(e){return e.map(t=>{let s=$(t),o=A(t);return`<li class="list_five_books modal-list" data-id="${t._id}">
  <a href="#" class='category-books__link'>
        <img class='category-books__img' src="${t.book_image}" alt="book" />
        <div class='category-books__wrapper'>
        <p class='category-books__text'>quick view</p>
  ...
  <button class="btn_back">BACK</button></div>`}function C(e){window.scrollTo(0,0);const t=F(e),s=J(e),o=G(s,t);g.insertAdjacentHTML("beforeend",o),document.querySelector(".btn_back").addEventListener("click",V)}function V(){location.reload()}K();const c={galleryTitle:document.querySelector(".list_general-title"),filterListEl:document.querySelector(".categories__list"),categoriesTitle:document.querySelector(".categories__title"),title:document.querySelector(".title_best_sellers")};L();function Q(e){j().then(t=>{const s=t.data;s.sort((o,r)=>o.list_name.localeCompare(r.list_name)),X(s)})}function X(e){const t=e.map(s=>`<li class="categories__list-item">${s.list_name}</li>`);c.filterListEl.insertAdjacentHTML("beforeend",t.join("")),E()}Q();c.filterListEl.addEventListener("click",Z);function Z(e){ee(e),L();const t=e.target.textContent;v(t).then(s=>{g.innerHTML="",C(s),E(),c.title.style.display="none"}).catch(s=>{Notiflix.Notify.failure("An error occurred while fetching images. Please try again.")})}function ee(e){if(e.target.tagName==="LI"){const t=c.filterListEl.querySelector(".selected-category");t&&t.classList.remove("selected-category"),e.target.classList.add("selected-category")}c.categoriesTitle.style.color="var(--text-color)"}S.defaults.baseURL="https://books-backend.p.goit.global/books/";async function _(e){const{data:t}=await S(e);return console.log(t),t}const y=document.querySelector("body"),n=[];document.addEventListener("click",te);function te(e){if(console.log(e),!e.target.closest(".modal-list"))return;console.log(e.target),e.preventDefault();const s=e.target.closest(".modal-list").dataset.id;console.log(s),_(s).then(o=>{console.log(n);const{_id:r,author:d,book_image:h,title:f,description:q,buy_links:[{url:x},{url:T}]}=o,b=W.create(`
